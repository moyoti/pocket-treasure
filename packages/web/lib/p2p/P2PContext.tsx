...
import { spawnService } from './spawn';
import {
  LocalIdentity,
  POI,
  SpawnedTreasure,
...
import { GachaEngine } from './engines/GachaEngine';
import { ChestEngine } from './engines/ChestEngine';
import { CosmeticEngine } from './engines/CosmeticEngine';
import { DailyTaskEngine } from './engines/DailyTaskEngine';
import { AchievementEngine } from './engines/AchievementEngine';
import { SellEngine, SellResult, SELL_PRICES } from './engines/SellEngine';
import { tradeService } from './trade/TradeService';
import { tradeEngine } from './trade/TradeEngine';
import { areaService } from './exploration/AreaService';
import { areaUnlockEngine } from './exploration/AreaUnlockEngine';
import { markerService } from './markers/MarkerService';
...
import { getRandomItemOfRarity } from './data/synthesis';
...
const areas = await databaseService.getVisitedAreas();
      ...
      const trades = await databaseService.getTradeHistory();
      ...
      if (typeof navigator !== 'undefined' && navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          (position) => {
            ...
            setUserLocation(loc);
            console.log('[P2P] Got user location:', loc);
          },
          (error) => {
            console.warn('[P2P] Geolocation error:', error.message);
